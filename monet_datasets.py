...
class Resize_shortest_side(object):
    
    def __init__(self, resize_):
        self.resize_ = resize_
    
    def __call__(self, img):
        
        if self.resize_ is None:
            return img 
        #[h,w]
        img = np.asarray(img)
        init_size = img.shape[:2]
        
        if np.min(init_size) >= self.resize_[0]:
            return img 
        
        short_side = np.min(init_size)
        ratio = self.resize_[0]/short_side
        h,w = int(init_size[0]*ratio), int(init_size[1]*ratio)
        if np.max([h,w]) >= self.resize_[1]:
            long_side = np.max(init_size)
            ratio = self.resize_[1] / long_side
            h,w = int(init_size[0]*ratio), int(init_size[1]*ratio)
...
class Dataset:
    def __init__(self, dataset_name, dataset_set, remove_hards,  patch_size=None, resize=None,):
        """
        ...
        if dataset_name == "VOC07":
            self.root_path = "/data_ssd/monet/datasets/VOC2007"
            self.year = "2007"
        elif dataset_name == "VOC12":
            self.root_path = "/data_ssd/monet/datasets/VOC2012"
            self.year = "2012"
        elif dataset_name == "VOCseg":
            self.root_path = '/data_ssd/monet/datasets/VOC2012'
            self.year = "2012"
            ...
            self.year = '2017'
            self.root_path = './datasets/COCOval/val2017'
            self.annfile = './datasets/COCOval/annotations/instances_val2017.json'
            self.val2017 = get_val2017(self.annfile)
        elif dataset_name == 'COCOtrain':
            self.year = '2017'
            self.annfile = '/data_ssd/monet/datasets/COCOval/annotations/instances_train2017.json'
            self.root_path = '/data_ssd/monet/datasets/COCOtrain'
            self.tain2017 = get_train2014(self.annfile)
        else:
        ...
        # Build the dataloader
        if dataset_name == 'VOCseg':
            self.dataloader = torchvision.datasets.VOCSegmentation(
                self.root_path,
                year=self.year,
                image_set=self.set,
                transform=transform,
        ...
        elif "COCOval" == dataset_name:
            self.dataloader = torchvision.datasets.CocoDetection(
                self.root_path, annFile=self.annfile, transform=transform
            )
        elif "COCOtrain" == dataset_name:
            self.dataloader = torchvision.datasets.CocoDetection(
                self.root_path, annFile=self.annfile, transform=transform
            )
    ...
    def load_image(self, im_name):
        ...
        elif "COCO20k" in self.dataset_name:
        ...
        elif "COCOval" in self.dataset_name:
            #im_path = self.path_20k[self.sel_20k.index(im_name)]
            #im_path = self.train2014['images'][self.sel_20k.index(im_name)]['file_name']
            #image = skimage.io.imread(f"./datasets/COCO/images/train2014/{im_path}")
            image = skimage.io.imread(os.path.join(self.root_path, im_name))
        elif "COCOtrain" in self.dataset_name:
            image = skimage.io.imread(os.path.join(self.root_path, im_name))
        else:
    ...
    def get_image_name(self, inp):
        ...
        if "VOC" in self.dataset_name:
            ...
            im_name = str(inp[0]["image_id"])
            im_name = '0'*(12 - len(im_name)) + im_name + '.jpg'
        elif "COCOtrain" in self.dataset_name:
            im_name = str(inp[0]["image_id"])
            im_name = '0'*(12 - len(im_name)) + im_name + '.jpg'
...
def get_val2017(all_annotations_file):
    # load all annotations
    with open(all_annotations_file, "r") as f:
        val2017 = json.load(f)
    return val2017
